...
function JiraIntegration({ onClose }) {
  ...
  if (loading) {
    return (
      <div className="w-full max-w-2xl mx-auto bg-[#09090b] text-white rounded-2xl border border-neutral-800 shadow-2xl p-16 flex flex-col items-center justify-center gap-4">
        <Loader2 className="w-9 h-9 text-white animate-spin" />
        <p className="text-sm font-medium text-neutral-400">Checking Jira connection...</p>
      </div>
    );
  }

  return (
    <div className="w-full max-w-2xl mx-auto bg-[#09090b] text-white rounded-2xl border border-neutral-800 shadow-2xl overflow-hidden font-sans flex flex-col">
      {/* Header Bar */}
      <div className="px-7 py-5 border-b border-neutral-800/90 flex items-center justify-between bg-[#0e0e11]">
        <div className="flex items-center gap-2.5">
          <h2 className="text-lg md:text-xl font-bold text-white tracking-tight">
            {showConnectionForm ? "Jira Integration" : "Project Details"}
          </h2>
          {!showConnectionForm && <Info size={18} className="text-neutral-400" />}
        </div>
        {onClose && (
          <button
            type="button"
            onClick={onClose}
            className="text-neutral-400 hover:text-white transition p-1.5 rounded-lg hover:bg-neutral-800"
          >
            <X size={20} />
          </button>
        )}
      ...
      {!showConnectionForm ? (
        <div className="flex flex-col bg-[#09090b]">
          {/* Sub Navigation Tabs */}
          <div className="flex items-center gap-8 px-7 pt-4 border-b border-neutral-800/80 bg-[#0e0e11]">
            <button
              type="button"
              onClick={() => {
                setActiveTab("all");
              ...
              className={`pb-3.5 text-sm font-semibold tracking-wide transition-all relative ${
                activeTab === "all" ? "text-white font-bold" : "text-neutral-400 hover:text-neutral-200"
              }`}
            >
              All Projects ({projects.length})
              {activeTab === "all" && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-white rounded-full" />}
            </button>
            ...
            <button
              ...
              className={`pb-3.5 text-sm font-semibold tracking-wide transition-all relative ${
                activeTab === "synced" ? "text-white font-bold" : "text-neutral-400 hover:text-neutral-200"
              }`}
            >
              Synced Projects ({selectedCount})
              {activeTab === "synced" && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-white rounded-full" />}
            </button>
          </div>

          {/* Filter Badges / Pills Bar */}
          <div className="flex items-center gap-3 px-7 py-4 border-b border-neutral-800/60 bg-[#09090b]">
            <button
              type="button"
              ...
              className={`flex items-center gap-2.5 px-4 py-2 rounded-xl text-xs md:text-sm font-semibold transition ${
                activeFilter === "selected"
                  ...
                  : "bg-[#141417] text-neutral-300 border border-neutral-800 hover:bg-[#1c1c20]"
              }`}
            >
              <span>Selected</span>
              <span
                className={`px-2 py-0.5 rounded-full text-xs font-bold ${
                  activeFilter === "selected" ? "bg-black text-white" : "bg-neutral-800 text-neutral-300"
                }`}
              >
                {selectedCount}
              </span>
            ...
            >
              ...
              <span
                className={`px-2 py-0.5 rounded-full text-xs font-bold ${
                  activeFilter === "integrated" ? "bg-black text-white" : "bg-neutral-800 text-neutral-300"
                }`}
              >
                {integratedCount}
              </span>
          ...
          <div className="px-7 pt-4 pb-2 space-y-3.5">
            ...
            <div className="flex items-center gap-3 bg-[#141417] border border-neutral-800 rounded-xl px-4 py-3 focus-within:border-neutral-600 transition">
              <Search size={18} className="text-neutral-400 shrink-0 pointer-events-none" />
              <input
                type="text"
                value={searchQuery}
            ...
            <div className="flex items-center px-1 py-1">
              <label className="flex items-center gap-3 text-sm font-semibold text-neutral-200 cursor-pointer select-none">
                <input
                  type="checkbox"
                  checked={allFilteredSelected}
                  onChange={handleSelectAllToggle}
                  className="w-4 h-4 rounded border-neutral-700 bg-neutral-800 accent-white cursor-pointer"
                />
              ...
              filteredProjects.map((project) => (
                ...
                >
                  <div className="flex items-center gap-4 min-w-0 pr-3">
                    ...
                    >
                      {project.isSelected && <Check size={14} strokeWidth={3} className="text-black" />}
                    </div>
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs md:text-sm font-medium text-neutral-400 mt-1 truncate">
                        {project.projectKey}
                      </p>
                    </div>
                  </div>
          ...
          {/* Footer Bar */}
          <div className="p-5 px-7 border-t border-neutral-800/90 flex items-center justify-between bg-[#0e0e11]">
            <button
              type="button"
              onClick={() => setShowConnectionForm(true)}
              className="px-6 py-2.5 text-sm font-semibold text-neutral-300 bg-[#18181b] border border-neutral-800 rounded-xl hover:bg-[#222226] hover:text-white transition"
            >
              Back To Integration
            </button>
            ...
            <button
              ...
              className="px-7 py-2.5 text-sm font-bold text-black bg-white hover:bg-neutral-200 rounded-full shadow-md transition active:scale-95 cursor-pointer"
            >
              Select Projects
            </button>
          </div>
        ...
        <form onSubmit={handleSubmitConnection} className="p-8 space-y-6 bg-[#09090b]">
          <div>
            <label className="block text-sm font-bold text-white mb-1">
              Jira Organisation Name
            </label>
            <p className="text-xs text-neutral-400 mt-1 mb-2.5">
              Get your Jira sitename from your Jira URL (e.g. <span className="text-neutral-300 font-mono">https://[sitename].atlassian.net</span>) and only enter the <span className="text-neutral-300 font-mono">[sitename]</span> portion.
            </p>
            <input
              type="text"
              name="hostName"
              ...
              className="w-full bg-[#141417] border border-neutral-800 rounded-xl px-4 py-3 text-sm text-white placeholder-neutral-500 focus:outline-none focus:border-neutral-600 transition"
              required
            />
          </div>

          <div>
            <label className="block text-sm font-bold text-white mb-1">API Token</label>
            <p className="text-xs text-neutral-400 mt-1 mb-2.5">
          ...
          <div>
            <label className="block text-sm font-bold text-white mb-1">Jira Login Email</label>
            <p className="text-xs text-neutral-400 mt-1 mb-2.5">
          ...
          <div className="pt-6 border-t border-neutral-800/80 flex items-center justify-between">
            {isConnected ? (
              <button
                ...
                className="px-5 py-2.5 text-sm font-semibold text-neutral-300 bg-[#18181b] border border-neutral-800 rounded-xl hover:bg-[#222226] hover:text-white transition"
              >
                Back To Projects
              </button>
            ) : (
            ...
            <button
              type="submit"
              disabled={connecting}
              className="flex items-center gap-2 px-7 py-2.5 text-sm font-bold text-black bg-white hover:bg-neutral-200 rounded-full shadow-md transition active:scale-95 cursor-pointer disabled:opacity-50"
            >
              {connecting && <Loader2 className="w-4 h-4 animate-spin text-black" />}
